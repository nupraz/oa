...
<script>
  import { fetchEmpAssessRecord,deptEmployees,saveAssessRecord } from '@/api/manage/employee'
  ...
  export default {
    ...
    methods: {
      ...
      selectMaxScore(){
            this.title = "员工评分平均分应在0-"+this.maxScore+"分"
      },
      startCreate(){
      ...
      getDate(){
        ...
        for(let i=0; i<this.tableData.length; i++){
          ...
          if(sc < 0){
            this.$message.error("第"+(i+1)+"行分数必须大于0分")
            return;
          }
        ...
        let average=sum/this.tableData.length;

        if(average> this.maxScore){
          this.$message.error("员工录入平均分数必须小于"+this.maxScore)
          return;
        }
         //存
