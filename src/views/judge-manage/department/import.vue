<template>
  <div class="app-container">
    <bv-table ref="table" title="部门绩效导入" :key="random" :pagination="false" :filter.sync="filter" :data="tableData"  @on-mounted="(table) => tableInstance = table" >
      <div slot="operates">
      ...
      <bv-table-column label="评分" prop="taskPerScore" align="center"/>
      <bv-table-column label="评委" prop="jurName" align="center"/>
      <bv-table-column label="协同评分" prop="cooperateScore" align="center"/>
      <bv-table-column label="扣分原因" prop="cooperateRemark" align="center"/>
      <bv-table-column label="时间" prop="manageTime" align="center"/>
    </bv-table>
    <bv-dialog title="调整平均分信息" :visible.sync="justdialogVisibleName" top="20vh" @close="cancelInsert">
      <bv-scrollbar style="height: 300px">
        <el-alert v-for="item in justmsg" :key="item.assessMonth" type="warning" :closable="false">
          <b>{{ item.assessMonth}}></b>    系统内平均分：<b>{{item.sysDeptAverage}}</b> 分  导入平均分：<b>{{item.importDeptAverage}}</b> 分   需要调整 <b>{{item.adjustScore}}</b> 分
        </el-alert>
      </bv-scrollbar>
      <div slot="footer">
    ...
    <el-dialog title="部门绩效信息导入" :visible.sync="exportDeptModal" width="30%">
      <el-upload
        ref="uploadFile"
...
<script>
  import {importDepFile,downloadTemplate,insertReport} from '@/api/report/statistic'
  ...
  export default {
    ...
    methods: {
      submitInsert(){
          let data = {
              "depreport":this.depreport,
            "justlist":this.justmsg,
            "recordlist":this.msg
          }
          insertReport(data).then(ret => {
      ...
      submitUpload(){
        ...
        importDepFile(file, data).then(ret => {
          //清除上传的文件
          this.$refs.uploadFile.clearFiles()
          ...
          if (code) {
            if(code == "9999"){
              ...
              this.depreport = ret.data.depreport
